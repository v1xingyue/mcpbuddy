...
| `list_hylo_assets` | 查询 Solana / Hylo 插件当前支持的资产、mint、状态和产品入口，例如 hyUSD、eHYUSD、hyloSOL、hyloSOL+、xSOL、cbBTC。 |
| `get_hylo_asset_balances` | 查询当前账户绑定 Solana 钱包中的 Hylo 资产余额；这是个人持仓查询，区别于 `list_hylo_assets` 的资产目录。 |
| `create_hylo_buy_asset` | 创建一笔通过 Jupiter 路由买入 Hylo 资产的未签名 Solana 交易，进入钱包审阅签名流程；服务端不签名、不自动广播。 |
| `create_hylo_sell_asset` | 创建一笔通过 Jupiter 路由卖出 Hylo 资产的未签名 Solana 交易，进入钱包审阅签名流程；金额使用 atomic integer。 |
