...
export function IdentityBindings({ providers }: { providers: string[] }) {
  ...
  return <section className="identity-bindings"><div><p className="eyebrow">SIGN-IN METHODS</p><h2>Bound identities</h2><p>Any bound method can sign in to this MCPBuddy account.</p></div><div className="binding-actions">{methods.map(method => {
    const isBound = bound.has(method.id);
    return <button className="identity-toggle ui-interactive-card" type="button" role="switch" aria-checked={isBound} key={method.id} onClick={() => toggle(method)} aria-label={`${method.label}: ${isBound ? 'connected, select to unbind' : 'not connected, select to bind'}`}><span className="identity-toggle-copy"><b>{method.label}</b><small>{isBound ? 'Connected' : 'Add connection'}</small></span><i aria-hidden="true" /></button>;
  })}{message && <p className="identity-message" role="status">{message}</p>}</div><dialog className="unbind-dialog" ref={dialog} onClose={() => setSelected(null)}><p className="label">UNBIND IDENTITY</p><h2>Remove {selected?.label}?</h2><p>You will no longer be able to sign in with this method. This does not delete your MCPBuddy data.</p><div><button className="quiet" type="button" onClick={() => dialog.current?.close()} disabled={pending}>Cancel</button><button className="unbind-confirm" type="button" onClick={confirm} disabled={pending}>{pending ? 'Removing…' : 'Unbind'}</button></div></dialog></section>;
}
