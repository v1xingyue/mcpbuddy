...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ swap?: string }> }) {
  ...
  const assetResult: SolanaPortfolio | { error: string } | null = wallet ? await getMainSolanaPortfolio(wallet.address, env.SOLANA_RPC_URL ?? 'https://api.mainnet-beta.solana.com').catch(error => ({ error: error instanceof Error ? error.message : 'Unknown Solana RPC error.' })) : null;
