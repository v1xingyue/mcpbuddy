...
import { authIdentities, users, walletBindings, walletTokenWatchlist } from '@/lib/db/schema';
...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ swap?: string }> }) {
  ...
  const db = getDb();
  const [[wallet], identities, duplicates, pendingSwaps, watchlist] = await Promise.all([
