...
import { createSolTransferForUser, createTokenTransferForUser, swapStatusForUser } from '@/lib/solana-swap';
...
export function registerSolanaBasePlugin(server: McpToolServer, context: McpPluginContext) {
  ...
  server.registerTool('create_solana_sol_transfer', { title: 'Create Solana SOL transfer', description: 'Create an unsigned native-SOL transfer for review and wallet signing. It never receives a private key or broadcasts.', inputSchema: { recipient: z.string().min(32).max(64), amount: z.string().regex(/^\d+(\.\d+)?$/).describe('Positive human-readable SOL amount, for example "0.1".') }, outputSchema: context.reviewOutputSchema, _meta: { 'openai/outputTemplate': context.reviewUiUri, 'ui/resourceUri': context.reviewUiUri } }, async (args: any, extra: any) => {
    try { const user = await context.currentUser(extra.authInfo?.extra?.githubId); const result = await createSolTransferForUser(user.id, args); const reviewUrl = `${context.appOrigin()}/account/wallet?swap=${result.transactionId}`; return { content: [{ type: 'text', text: `Unsigned ${result.summary.inputAmount} SOL transfer created for ${args.recipient}. Open ${reviewUrl} to review and sign.` }], structuredContent: { ...result, reviewUrl, signingRequired: true }, _meta: { 'openai/outputTemplate': context.reviewUiUri } }; } catch (error) { return { content: [{ type: 'text', text: error instanceof Error ? error.message : 'Could not create SOL transfer.' }], isError: true }; }
  });
